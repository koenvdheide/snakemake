...
Information of the interpretation hereof can be found [here](https://www.biostars.org/p/3920/). Also you may be more familiar with the textual output which can be found [here](data/blast_results.out.txt) 
...
As seeing the downloaded sequences is not that useful we refer to the [FASTA file](data/uniprot_sequences.txt) of these and chose to not include these in the report directly.
...
```{r echo = FALSE}
literature.table <- read.table('data/literature_tab.txt',sep = '\t', header = F, fill = T)
...
```{r echo = FALSE}
kegg.data <- read.table("data/kegg_table.txt", sep = '\t', header = F)
